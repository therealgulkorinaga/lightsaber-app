/**
 * Lightsaber — Carbon-inspired monoline icon set
 * ──────────────────────────────────────────────────────────────────────
 * 45 icons · 24×24 grid · stroke 1.5 · square caps · miter joins
 * Strokes only. Exception: `more` uses filled squares.
 *
 * Plain-JS / browser-globals version (no JSX, no TS) so it can be
 * loaded with `<script src="assets/icons.js"></script>` and used from
 * a Babel-compiled component file via the `Icon` global.
 *
 * Path data is identical to handoff/icons-react.tsx.
 */
...
(function () {
  const ICON_PATHS = {
    ...
    // Affordance — `more` is the one filled icon
  ...
  /**
   * Render an icon. Returns an SVG string OR (if React is loaded)
   * a React element via React.createElement when called with `.r(...)`.
   *
   *   Icon('check', { size: 16, color: 'currentColor' })  → SVG string
   *   Icon.r('check', { size: 16 })                       → React element
   *
   * Always sets stroke 1.5, linecap square, linejoin miter.
   */
  function svgString(name, opts) {
    ...
    return (
      ...
      ' style="flex:0 0 auto;display:inline-block;vertical-align:middle"' +
  ...
  function ReactIcon(props) {
    ...
    if (!R) return null;
    const { name, size = 16, color = 'currentColor', strokeWidth = 1.5, style, className, ...rest } = props;
    const paths = ICON_PATHS[name];
  ...
  svgString.r = ReactIcon;
  svgString.paths = ICON_PATHS;
  svgString.names = Object.keys(ICON_PATHS);

  window.Icon = svgString;
  window.LightsaberIcons = ICON_PATHS;
})();
